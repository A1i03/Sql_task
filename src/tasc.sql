...
select g.id , s.first_name, s.email, s.gender from students s
                                                       inner join groups g on g.id = s.group_id
where g.id = 2;

--Вывести группу студента, айди которого равна 4
select * from students s where s.id =4;
...
update students set first_name = 'ali', last_name = 'Kaldarbekov', gender = 'Male', email = 'ali@gmail.com', date_of_birth ='14-04-2003' where students.id = 8;
...
select min(s.date_of_birth) from courses c
...
alter table mentors add check (gender in ('Male', ' ', 'Female'));

--Добавьте check constraint gender в столбец таблицы students Tasks(Query) Mentors
alter table students add check (gender in ('Male', ' ', 'Female'));
...
select  m.first_name, m.email, m.specialization , group_name
from mentors m
...
select count(s.id), m.first_name from  mentors m
                                           inner join courses c on c.id = m.course_id
                                           inner join groups g on g.id = c.groups_id
                                           inner join students s on g.id = s.group_id group by m.first_name;
...
select * from mentors m
                  inner join lessons l on m.course_id = l.course_id
where m.id = 5
;
...
select count(l.id), g.group_name from lessons l
                                          inner join courses c on c.id = l.course_id
                                          inner join groups g on g.id = c.groups_id group by g.group_name having count(l.id)>2;
...
select * from lessons l
                  ...
                  inner join lessons l2 on c.id = l2.course_id
where m.id = 5;
